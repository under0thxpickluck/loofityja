...
**Goal:** `/lifai/guide` に日本語ハードコードのLIFAI換金お客様向けガイドページを新規作成する

**Architecture:** `app/[locale]/lifai/guide/page.tsx` 単一のServer Componentに全コンテンツをハードコード。i18n翻訳キー不使用、ログイン不要の公開ページ。既存 `/guide` ページのTailwindスタイルパターンを踏襲し、インラインSVGでイラストを表現する。

**Tech Stack:** Next.js 14 App Router, Tailwind CSS, `Link` from `@/i18n/navigation`, `ReactNode` / `JSX` from react
...
| ファイル | 操作 | 役割 |
|---------|------|------|
| `app/[locale]/lifai/guide/page.tsx` | 新規作成 | 全セクション（Hero / フロー概要 / 詳細ステップ / FAQ / CTA）を含む単一Server Component |

---

### Task 1: ページシェル — Hero + フロー概要

**Files:**
- Create: `app/[locale]/lifai/guide/page.tsx`

- [ ] **Step 1: ファイルを作成する**

`app/[locale]/lifai/guide/page.tsx` を以下の内容で作成する。
...
期待: エラーなし（または既存の無関係なエラーのみ）

- [ ] **Step 3: コミット**

```bash
git add "app/[locale]/lifai/guide/page.tsx"
git commit -m "feat: LIFAI換金ガイド — Hero + フロー概要セクションを追加"
```

---

### Task 2: インラインSVGイラスト + 詳細ステップセクション

**Files:**
- Modify: `app/[locale]/lifai/guide/page.tsx`

- [ ] **Step 1: SVGイラストコンポーネントとDETAIL_STEPSデータを追加する**

`// Flow overview icons` ブロックの**直前**（ファイル先頭 import の直後）に以下を挿入する。
...
- [ ] **Step 2: DETAIL_STEPSデータを追加する**

`// Data: フロー概要` ブロックの**直前**に以下を挿入する。
...
const DETAIL_STEPS: Array<{ title: string; body: ReactNode; Illustration: () => JSX.Element }> = [
  ...
  {
    title: '申請してEPを送金',
    body: '申請ボタンを押すと申請番号と運営ウォレットアドレスが表示されます。表示されたウォレットアドレスへ申請したEP数量を正確に送金してください。申請後はなるべくお早めに送金をお願いします。',
...
- [ ] **Step 3: 詳細ステップセクションをrenderに追加する**

`{/* ── フロー概要 ── */}` セクションの**直後**（`</div>` の閉じタグの後）、`</div>` (ルートdiv) の**直前**に挿入する。
...
```bash
git add "app/[locale]/lifai/guide/page.tsx"
git commit -m "feat: LIFAI換金ガイド — SVGイラスト付き詳細ステップセクションを追加"
```
...
- [ ] **Step 2: FAQセクションとCTAバナーをrenderに追加する**

`{/* ── 詳細ステップ ── */}` セクションの**直後**、ルートdivの閉じタグ `</div>` の**直前**に挿入する。
...
```bash
git add "app/[locale]/lifai/guide/page.tsx"
git commit -m "feat: LIFAI換金ガイド — FAQ + CTAセクションを追加して完成"
```
...
- [ ] **Step 1: プロダクションビルドを確認する**

```powershell
npx next build 2>&1 | Select-Object -Last 30
```

期待: `Route (app)` に `/[locale]/lifai/guide` が表示され、エラーなし。
