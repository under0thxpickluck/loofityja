...
**Architecture:** Next.js APIルート（`/api/lifai/deposit-status`）が判定ロジックの中心。LIFAI GASから入金一覧を取得し、純粋関数 `evaluateDeposits` で判定、確認成立時はLIFAI側で入金を `consumed` にマークしてから Lootify GAS 経由でシートへ書き戻す。「未消費 + 当該申請IDで消費済み」を合計するため書き戻し失敗後のリトライも冪等。

**Tech Stack:** Next.js 14 (App Router, route handlers), Google Apps Script ×2 (Lootify / LIFAI), jest + ts-jest, next-intl
...
- LIFAI GAS = `C:\Users\unitu\LIFAI\gas\Code.gs`（別プロジェクト）。EP残高と `lfw_deposits` シートを持つ。`LIFAI_GAS_URL` + `?key=<LIFAI_GAS_KEY>` で呼ぶ。
...
### Task 1: LIFAI側GAS — consume_lfw_deposits アクション追加

**Files:**
- Modify: `C:\Users\unitu\LIFAI\gas\Code.gs`（dispatch行 ~7608付近、checkLfwDeposit_ ~12850付近、ファイル末尾 ~12862）
...
- [ ] **Step 1-4: コミット（LIFAIがgitリポジトリの場合のみ）**

Run: `git -C C:\Users\unitu\LIFAI rev-parse --is-inside-work-tree`
リポジトリなら:
```bash
git -C C:\Users\unitu\LIFAI add gas/Code.gs
git -C C:\Users\unitu\LIFAI commit -m "feat: LFW入金の消費マークアクション(consume_lfw_deposits)を追加"
```
...
### Task 3: LIFAI GASクライアント（サーバー専用）

**Files:**
- Create: `lib/LIFAI-client.ts`
...
/**
 * LIFAI側GASのクライアント。サーバー専用（環境変数 LIFAI_GAS_URL / LIFAI_GAS_KEY を使用）。
 * クライアントコンポーネントからimportしないこと。
 */
async function callLIFAI<T>(payload: Record<string, unknown>): Promise<T | null> {
  const gasUrl = process.env.LIFAI_GAS_URL
  const gasKey = process.env.LIFAI_GAS_KEY
  if (!gasUrl || !gasKey) {
    console.error('[LIFAI-client] LIFAI_GAS_URL / LIFAI_GAS_KEY is not set')
    return null
  }
  try {
    ...
    console.error('[LIFAI-client] request failed:', err)
    return null
  }
}

export type LIFAICheckResult = { ok: boolean; deposits?: LfwDeposit[]; error?: string }
export type LIFAIConsumeResult = { ok: boolean; consumed?: number; error?: string }

export function checkLfwDeposits(lfwAddress: string): Promise<LIFAICheckResult | null> {
  return callLIFAI<LIFAICheckResult>({ action: 'check_lfw_deposit', lfw_address: lfwAddress, min_amount: 0 })
}
...
export function consumeLfwDeposits(
  ...
  depositIds: string[],
  consumedBy: string
): Promise<LIFAIConsumeResult | null> {
  return callLIFAI<LIFAIConsumeResult>({
...
```bash
git add lib/LIFAI-client.ts
git commit -m "feat: LIFAI GASクライアント(check/consume)を追加"
```
...
# LIFAI側GAS（入金確認・消費）— サーバー専用
LIFAI_GAS_URL=https://script.google.com/macros/s/your-LIFAI-web-app-id/exec
LIFAI_GAS_KEY=your-LIFAI-secret
...
git commit -m "docs: LIFAI/LOOTIFY GAS用の環境変数をexampleに追記"
```

- [ ] **Step 13-5: ユーザー向けデプロイ手順を最終報告に含める**

実装完了報告に以下を必ず記載:
1. `.env.local` に `LIFAI_GAS_URL` / `LIFAI_GAS_KEY`（Vercelからコピー）と `LOOTIFY_GAS_KEY`（新規生成した強いランダム文字列）を追加
...
4. LIFAI GAS: `C:\Users\unitu\LIFAI\gas\Code.gs` の内容に置き換え → 再デプロイ
5. エンドツーエンド確認: 申請 → LIFAIからEP送付（少なめ）→ 不足表示 → 残りを送付 → 確認済み → スプレッドシートの status / 受領EP合計 / 充当入金ID を確認
