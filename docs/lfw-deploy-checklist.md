...
- [ ] **2. `.env.local` に追記**（ローカル開発用）
  - `LIFAIOV_GAS_URL` — Vercelに設定済みの値をコピー
  - `LIFAIOV_GAS_KEY` — Vercelに設定済みの値をコピー
...
- [ ] **4. Lootify GAS を更新**
  ...
  - 再デプロイ
- [ ] **5. LIFAIOV GAS を更新**
  - GASエディタの内容を `C:\Users\unitu\lifaiov\gas\Code.gs` で置き換え
  - 再デプロイ
...
- [ ] LIFAIOVから申請額より**少なめ**にEPを送付 → 30秒以内に「不足: 受領◯EP / ◯EP不足」表示になる
...
- [ ] `lib/lifaiov-client.ts` のキー送信をbody単独にする（現状クエリ文字列にも含まれGASログに残る可能性）
